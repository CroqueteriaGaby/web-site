...
function DownloadPDFButton() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const [{ pdf }, { default: CatalogPDF }, logoBase64] = await Promise.all([
      ...
      const imageCache = await preloadMissingImages(catalogData, domCache, (loaded, total) =>
      ...
      const blob = await pdf(
        <CatalogPDF catalogData={catalogData} imageCache={imageCache} logoBase64={logoBase64} />,
      ).toBlob();
